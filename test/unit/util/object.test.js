...
test('object', async (t) => {
    ...
    await t.test('map', (t) => {
        t.assert.deepEqual(mapObject({}, () => { t.assert.fail(); }), {});
        const that = {};
        t.assert.deepEqual(mapObject({ map: 'box' }, function (value, key, object) {
    ...
    await t.test('filter', (t) => {
        t.assert.deepEqual(filterObject({}, () => { t.assert.fail(); }), {});
        const that = {};
        ...
        t.assert.deepEqual(filterObject({ map: 'box', box: 'map' }, (value) => {
            return value === 'box';
        }), { map: 'box' });
    });
